...
if __name__=="__main__":   
    ...
    n_exp = 1000           # Number of realizations
    exec_times = np.zeros((n_exp,3))
    ...
    for i in range(n_exp):       
        ...
        Af = A.T.copy().astype(np.float32)                  # Here is the key
        t_start = time.time()
        ...
        exec_times[i,:]= np.array([t_numpy, t_jit, t_vector])*1e3
    avg_exec_times=np.mean(exec_times, axis=0)

    print(f'\n%%%%\nFinished execution\n')
    if np.allclose(y_numpy,y_jit) and np.allclose(y_numpy,y_vector.astype(int)):
        print(f'All the results are correct\nAverage elapsed time in milliseconds:')
        print('    Numpy: {:0.3f} '.format(avg_exec_times[0]))
        print('    JIT: {:0.3f} '.format(avg_exec_times[1]))
        print('    Vectorization: {:0.3f} '.format(avg_exec_times[2]))
