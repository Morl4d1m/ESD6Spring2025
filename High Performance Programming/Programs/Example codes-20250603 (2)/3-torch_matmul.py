...
def fill_torch(N,K,M,n_exp):
    x = torch.randint(M, size = (K,N)).double()     # Filling vector
...
def fill_np(N,K,M,n_exp):
    x = np.float64(np.random.randint(M, size = (K,N)))     # Filling vector
...
if __name__=="__main__":   
    ...
    for i in range(n_exp):       
        ...
        t0 = time.time()
        y_gpu = torch.matmul(x_gpu,A_gpu)
        if torch.cuda.current_device()==0:
            torch.cuda.synchronize()
        else:
            y_cuda = y_gpu
