...
bool is_valid_fft_size(size_t n) {
  return n == 16 || n == 64 || n == 256 || n == 1024 || n == 4096 || n == 16384 || n == 65536;
}
...
__attribute__((noinline)) void computeImpulseResponse() {
  ...
  const size_t max_samples = 8192;  // Safe for Teensy 4.1 with 8MB PSRAM
  ...
  if (!is_valid_fft_size(Nfft)) {
    Serial.println("ERROR: Invalid FFT size for arm_cfft_radix4_f32");
    return;
  }

  float* X = (float*)extmem_malloc(2 * Nfft* sizeof(float));
  float* Y = (float*)extmem_malloc(2 * Nfft* sizeof(float));
  float* H = (float*)extmem_malloc(2 * Nfft* sizeof(float));
  float* h = (float*)extmem_malloc(Nfft * sizeof(float));
...
for (size_t i = 0; i < Nfft * 2; i++) {
  if (!isfinite(X[i])) {
    Serial.printf("Invalid value at X[%u] = %f\n", i, X[i]);
    return;
  }
}
  ...
  float* Hfreq = (float*)extmem_malloc(2 * Nfft * sizeof(float));
  if (!Hfreq) {
