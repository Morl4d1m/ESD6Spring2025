...
// Global variables
uint16_t count = 1;
const uint8_t ledPin = 2;        // Pin 13 is the builtin LED
uint32_t LFSRBits = 28;           // Change this between 2 and 32
...
void generateMLS() {
  ...
  uint32_t MLSLength = (1UL << LFSRBits) - 1;  // Maximum value for 32-bit unsigned integer
  uint32_t increment = 1048576;           // The amount by which the loop increments

  for (uint32_t start = 0; start < MLSLength; start += increment) {
    uint32_t end = start + increment;
    if (end > MLSLength) {
      end = MLSLength;  // Ensure the loop does not exceed MLSLength
    }
    ...
    //Serial.println("Next");  // After each chunk, print "Next"
  }
