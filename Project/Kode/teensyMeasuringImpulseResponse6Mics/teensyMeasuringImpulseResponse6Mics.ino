...
void recordSine() {
  ...
  for (int f = 20; f <= 1220; f += 50) {  // Increments frequency by 50 Hz
    ...
    if (iteration == 0) {
      snprintf(filenameCombined, sizeof(filenameCombined), "sine%dCombined6MicsCH12.csv", f);
    } else if (iteration == 1) {
      snprintf(filenameCombined, sizeof(filenameCombined), "sine%dCombined6MicsCH34.csv", f);
    } else if (iteration == 2) {
      snprintf(filenameCombined, sizeof(filenameCombined), "sine%dCombined6MicsCH56.csv", f);
    } else {
...
void recordPhaseShift() {
  ...
  for (int phase = 0; phase <= 360; phase += 90) {  // Increments phase shift by 90 degrees
    ...
    char filenameCombined[45];
    if (iteration == 0) {
      snprintf(filenameCombined, sizeof(filenameCombined), "sineShifted%dCombined6MicsCH12.csv", phase);
    } else if (iteration == 1) {
      snprintf(filenameCombined, sizeof(filenameCombined), "sineShifted%dCombined6MicsCH34.csv", phase);
    } else if (iteration == 2) {
      snprintf(filenameCombined, sizeof(filenameCombined), "sineShifted%dCombined6MicsCH56.csv", phase);
    } else {
      Serial.println("Unknown iteration number - Shutting down!");
      while (true)
        ;
...
void recordWhiteNoise() {
  ...
  removeIfExists("whiteNoiseCombined6Mics.csv");  // Deletes previous versions of the file, so that a new file is created, ensuring data integrity
  if (iteration == 0) {
    combinedFile = SD.open("whiteNoiseCombined6MicsCH12.csv", FILE_WRITE);  // Creates file on the SD
  } else if (iteration == 1) {
    combinedFile = SD.open("whiteNoiseCombined6MicsCH34.csv", FILE_WRITE);  // Creates file on the SD
  } else if (iteration == 2) {
    combinedFile = SD.open("whiteNoiseCombined6MicsCH56.csv", FILE_WRITE);  // Creates file on the SD
  } else {
    Serial.println("Unknown iteration number - Shutting down!");
    while (true)
      ;
...
void recordSineSweep() {
  ...
  removeIfExists("sweepCombined6Mics.csv");  // Deletes previous versions of the file, so that a new file is created, ensuring data integrity
  if (iteration == 0) {
    combinedFile = SD.open("sweepCombined6MicsCH12.csv", FILE_WRITE);  // Create file on SD card
  } else if (iteration == 1) {
    combinedFile = SD.open("sweepCombined6MicsCH34.csv", FILE_WRITE);  // Create file on SD card
  } else if (iteration == 2) {
    combinedFile = SD.open("sweepCombined6MicsCH56.csv", FILE_WRITE);  // Create file on SD card
  } else {
    Serial.println("Unknown iteration number - Shutting down!");
    while (true)
      ;
